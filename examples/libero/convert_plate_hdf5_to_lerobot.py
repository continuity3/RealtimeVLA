"""
转换 plate 数据集的 HDF5 格式数据到 LeRobot 格式的脚本。

这个脚本专门用于处理 plate 数据集的 HDF5 文件。
...
Usage:
uv run examples/libero/convert_plate_hdf5_to_lerobot.py --data_dir /path/to/plate
"""
...
REPO_NAME = "your_hf_username/plate_libero"  # 输出数据集名称
...
def load_plate_hdf5(hdf5_path: Path, task_description: str = "Plate manipulation task", ignore_valid: bool = False) -> list[dict]:
    """
    从 plate HDF5 文件中加载数据。
...
def main(
    ...
    *,
    ...
    task_description: str = "Plate manipulation task",
    ignore_valid: bool = False,
):
    """
    主函数：将 plate HDF5 格式数据转换为 LeRobot 格式
    ...
    for hdf5_path in tqdm(hdf5_files, desc="处理 HDF5 文件"):
        try:
            steps = load_plate_hdf5(hdf5_path, task_description, ignore_valid)
    ...
    if push_to_hub:
        dataset.push_to_hub(
            tags=["libero", "panda", "plate", "manipulation"],
            private=False,
            push_videos=True,
            license="apache-2.0",
        )
